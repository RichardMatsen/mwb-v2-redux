...
export abstract class DataService {
  ...
  public initializeList(numToInitialize, numToDisplay) {
    ...
    this.getContentForList$(ordered$, numToInitialize)
      .subscribe(
  ...
  public updateList(numToDisplay: number) {
    ...
    const source$ = this.files$
      ...
      .concatMap(files => files); // convert to observable of list items
    this.getContentForList$(source$, numToDisplay)
      .subscribe(
  ...
  private getContentForList$(source$: Observable<IFileInfo>, numToInitialize: number): Observable<IFileInfo[]> {
    const withContent$ = source$
      // .take(numToInitialize)
      .concatMap((file, index) => file.content || index >= numToInitialize ? Observable.of(file) : this.getContent(file))   // Use concatMap to preserve ordering
